...
- LLM: **Google Gemini, `gemini-3.5-flash-lite`, swapped in as the default provider on 2026-09-10.** Originally scoped as Anthropic Claude Sonnet on the strength of the developer's stated API credits; those credits turned out to be exhausted ($0 usable balance confirmed via a live smoke test against the real API). Rather than block on billing, switched the default to Gemini, which has a genuine standing free tier — exactly the constraint the assessment brief itself calls out. The adapter-pattern design (`LlmAdapter` interface, RFC section 3.1) made this a same-day, low-risk swap: `ClaudeAdapter` remains implemented and available via `LLM_PROVIDER=anthropic` for whenever credits are added, but `GeminiAdapter` is the default. The model itself was also revised mid-build: the initial pick, `gemini-3.6-flash`, has a free tier of only 5 requests/minute and 20/day — checked directly against the AI Studio rate-limit dashboard after live batch testing hit 429s — which is too tight for a pipeline that makes several calls per kit. Every "Flash Lite" variant across model generations carries a materially better free tier (15 RPM / 500 RPD), so the default moved to `gemini-3.5-flash-lite`. Verified end-to-end against live Gemini + the local fixture site via the actual mandated batch command (`npm run evaluate`, not a smoke test): 3 cases — a normal JD, a thin JD, and an unreachable company URL — all completed "ok" in 11 seconds, each handled honestly (full coverage on the normal case, nothing invented on the thin JD, an honestly-empty brief on the unreachable site).
...
## 10. Decisions (resolved at approval)

These were refinements, not disagreements — the five experts converged on the overall design. Resolved as follows at RFC approval (2026-09-10), each defaulting to the panel's own recommendation:

1. **Backend host: Render.** Free-tier persistent Node process, simplest config of the candidates; cold-start behavior will be verified early (day 1 throwaway deploy per Tech Lead's milestone plan) and documented in the README as a known limitation if it materializes rather than switching hosts mid-build.
2. **Password hashing: bcrypt** (cost factor ≥12). Chosen over argon2id for the safer native-binding story on constrained free-tier build environments — Security flagged this as the practical tie-breaker.
3. **CSRF strategy: Origin/Referer-header verification** on all state-changing requests, paired with the `SameSite=None`+`Secure` session cookie. Lighter-weight than a double-submit token and sufficient given the backend already validates the request origin against the CORS allow-list.
4. **Coverage-loop max passes: 2** (first draft + one gap-fill), per Architect/QA convergence. Revisit only if real-world batch timing leaves comfortable headroom under the 15-minute/5-case ceiling.
5. **Duplicate-submission UX**: on `409 DUPLICATE_KIT`, show the user a choice — "open existing kit" or "create anyway" (`force: true`) — rather than silently auto-navigating, so the user isn't surprised by a kit they didn't ask to open.
6. **Session collection**: confirmed as part of the data model (§6.2) — a `Session` collection backing the server-side session store, filling the gap between Design Engineer's initial schema draft and Security's recommended session approach.
...
## Developer Sign-off
Approved by: Sourav  Date: 2026-09-10
Notes: Approved with open questions (§10) resolved to panel defaults at approval time (Render, bcrypt, Origin/Referer CSRF check, 2-pass coverage cap, explicit duplicate-submission choice UX, Session collection confirmed).
